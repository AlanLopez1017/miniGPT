...
class Trainer:
    def __init__(self, model, data_loader, config):
        ...
        self.log_dir = os.path.join(self.config.checkpoint_dir, "logs")
    ...
    def train(self):
        ...
        for iter in range(self.config.max_iters):
            ...
            self._append_step_loss(iter, step_loss)
            ...
            if iter % self.config.eval_interval == 0:
                ...
                self._append_eval_losses(iter, losses["train"], losses["val"])
                self.save_history()
        ...
        self._append_eval_losses(self.config.max_iters, final_losses["train"], final_losses["val"])
        self.save_history()
...
def save_text_output(text: str, out_path: str, header: str = ""):
...
def main():
    ...
    after_train = generate_sample(gpt, data_loader, max_tokens=500, device=config.device)
    ...
    after_out_path = os.path.join(config.checkpoint_dir, f"{config.model_name}_output_after_training.txt")
    save_text_output(after_train, after_out_path, header="Model output (after training)")
    print(f"Saved: {after_out_path}")
...
def test_model(checkpoint_path, data_loader, stoi, vocab_size, device, prompt = None, max_tokens = 500):
    ...
    out_path = os.path.join(config.checkpoint_dir, f"{config.model_name}_test_output.txt")
    header = f"Model output (test)\nPrompt: {prompt}\nMax tokens: {max_tokens}"
    save_text_output(result, out_path, header=header)
    print(f"Saved: {out_path}")

def test_saved_model(prompt, max_tokens = 500):
    """Test saved model"""
    ...
    test_model(checkpoint_path, data_loader, stoi, vocab_size, config.device, prompt, max_tokens)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--test':
        ...
        max_tokens = 500
        test_saved_model(prompt, max_tokens)
    else:
        # Training model
        main()
